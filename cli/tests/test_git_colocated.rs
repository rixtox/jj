...
fn test_workspace_add_colocate_git_status_and_diff() {
    ...
    // and confirm `git` sees EXACTLY that change (not the wrong index / not all files).
...
fn test_workspace_add_colocate_worktree_list_and_common_dir() {
    ...
    // the test tmpdir is a `/var -> /private/var` symlink; if realpath handling were
    // wrong Git would report the worktree as `prunable`, so this also guards AC11.
...
fn test_workspace_add_colocate_merge_working_copy() {
    ...
    work_dir
        .run_jj(["workspace", "add", "../second", "-r", &commit_a, "-r", &commit_b])
        .success();
...
fn test_workspace_add_colocate_conflicted_working_copy() {
    ...
    assert!(
        jj_status.stdout.normalized().to_lowercase().contains("conflict")
            || jj_status.stderr.normalized().to_lowercase().contains("conflict"),
...
fn test_workspace_add_colocate_undo() {
    ...
    // AC14: `jj undo` reverses the op-store side of `workspace add`. The Git worktree
    // admin files were written OUTSIDE the transaction, so they are not reversed
    // (they persist, now orphaned); the important guarantee is that jj does not crash.
    let output = work_dir.run_jj(["undo"]);
    assert!(
