...
impl GitBackend {
    ...
    pub fn init_internal(
        ...
        Self::init_with_repo(
            store_path,
            git_repo_path,
            MaybeColocatedGitRepo { git_repo },
            git_settings,
        )
    }
...
mod tests {
    ...
    fn read_plain_git_commit(object_hash: gix::hash::Kind) -> TestResult {
        ...
        let backend = GitBackend::init_external(&settings, store_path, git_repo.path(), None)?;
    ...
    fn round_trip_change_id_via_git_header(object_hash: gix::hash::Kind) -> TestResult {
        ...
        let no_extra_backend =
            GitBackend::init_external(&settings, &empty_store_path, git_repo.path(), None)?;
